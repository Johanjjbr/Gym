# 🏗️ ARQUITECTURA DEL SISTEMA

## Sistema de Gestión de Gimnasio Los Teques

---

## 📐 ARQUITECTURA GENERAL

```
┌─────────────────────────────────────────────────────────────────┐
│                        FRONTEND (React + TypeScript)            │
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │  Dashboard   │  │    Users     │  │   Payments   │         │
│  └──────────────┘  └──────────────┘  └──────────────┘         │
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │  Routines    │  │  Attendance  │  │    Staff     │         │
│  └──────────────┘  └──────────────┘  └──────────────┘         │
│                                                                 │
│  ┌────────────────────────────────────────────────────┐        │
│  │         API Client (/src/app/lib/api.ts)           │        │
│  │  - auth.login()                                     │        │
│  │  - users.getAll()                                   │        │
│  │  - payments.create()                                │        │
│  │  - stats.getDashboard()                             │        │
│  └────────────────────────────────────────────────────┘        │
└─────────────────────────────────────────────────────────────────┘
                              ↓ HTTPS
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                   SUPABASE EDGE FUNCTIONS                       │
│                  (Hono Server - Deno Runtime)                   │
│                                                                 │
│  ┌────────────────────────────────────────────────────┐        │
│  │         /supabase/functions/server/index.tsx        │        │
│  │                                                     │        │
│  │  Endpoints:                                         │        │
│  │  - POST /auth/login                                 │        │
│  │  - GET  /users                                      │        │
│  │  - POST /payments                                   │        │
│  │  - GET  /stats                                      │        │
│  │  - ... (todos los endpoints CRUD)                  │        │
│  └────────────────────────────────────────────────────┘        │
│                                                                 │
│  🔐 Autenticación: Supabase Auth (JWT Tokens)                  │
│  🔒 Seguridad: Row Level Security (RLS)                        │
└─────────────────────────────────────────────────────────────────┘
                              ↓ PostgreSQL Protocol
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                    SUPABASE DATABASE                            │
│                      (PostgreSQL)                               │
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐       │
│  │  users   │  │  staff   │  │ payments │  │attendance│       │
│  │  (12)    │  │  (3)     │  │  (2)     │  │  (0)     │       │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘       │
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐       │
│  │physical_ │  │routine_  │  │exercise_ │  │user_rout.│       │
│  │progress  │  │templates │  │templates │  │assignm.  │       │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘       │
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐       │
│  │workout_  │  │workout_  │  │ set_logs │  │ invoices │       │
│  │sessions  │  │exercise_ │  │          │  │          │       │
│  │          │  │logs      │  │          │  │          │       │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘       │
│                                                                 │
│  🔐 Row Level Security (RLS) activo en todas las tablas        │
│  🔑 Políticas por rol: Admin, Entrenador, Recepción           │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🔄 FLUJO DE DATOS

### Login Flow

```
┌─────────┐                                      ┌─────────┐
│ Usuario │                                      │ Sistema │
└────┬────┘                                      └────┬────┘
     │                                                 │
     │  1. Ingresa email y password                   │
     │ ─────────────────────────────────────────────> │
     │                                                 │
     │                                          [Frontend]
     │                                          api.auth.login()
     │                                                 │
     │                                                 ▼
     │                                          POST /auth/login
     │                                                 │
     │                                          [Backend Server]
     │                                          Verifica credenciales
     │                                          con Supabase Auth
     │                                                 │
     │                                                 ▼
     │                                          SELECT * FROM staff
     │                                          WHERE auth_user_id = ?
     │                                                 │
     │  2. Retorna token + datos de staff             │
     │ <───────────────────────────────────────────── │
     │                                                 │
     │                                          [Frontend]
     │                                          Guarda token en localStorage
     │                                          Guarda user en localStorage
     │                                                 │
     │  3. Redirige al Dashboard                      │
     │ <───────────────────────────────────────────── │
     │                                                 │
```

### CRUD Flow (Ejemplo: Obtener Usuarios)

```
┌────────────┐                                    ┌────────────┐
│  Frontend  │                                    │  Backend   │
└─────┬──────┘                                    └─────┬──────┘
      │                                                  │
      │  1. api.users.getAll()                          │
      │ ──────────────────────────────────────────────> │
      │     Headers:                                     │
      │     Authorization: Bearer <token>                │
      │                                                  │
      │                                           [Servidor]
      │                                           Verifica token
      │                                                  │
      │                                                  ▼
      │                                           SELECT * FROM users
      │                                           ORDER BY created_at DESC
      │                                                  │
      │                                           [RLS Check]
      │                                           ¿Usuario tiene permiso?
      │                                                  │
      │  2. Retorna array de usuarios                   │
      │ <────────────────────────────────────────────── │
      │     [{ id, name, email, ... }, ...]             │
      │                                                  │
      │  3. Actualiza UI                                │
      │                                                  │
```

---

## 🔐 SISTEMA DE SEGURIDAD

### Capas de Seguridad

```
┌─────────────────────────────────────────────────────────────────┐
│  CAPA 1: Autenticación con Supabase Auth (JWT)                 │
│                                                                 │
│  - Tokens firmados con clave secreta                           │
│  - Expiración automática                                       │
│  - Refresh tokens para sesiones largas                         │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  CAPA 2: Validación en Servidor                                │
│                                                                 │
│  - Verificación de token en cada request                       │
│  - Consulta de datos del staff desde BD                        │
│  - Validación de permisos por endpoint                         │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  CAPA 3: Row Level Security (RLS) en PostgreSQL                │
│                                                                 │
│  - Políticas aplicadas a nivel de base de datos                │
│  - Filtrado automático según rol del usuario                   │
│  - Protección contra SQL injection                             │
└─────────────────────────────────────────────────────────────────┘
```

### Matriz de Permisos Detallada

```
┌──────────────────────┬─────────────┬─────────────┬─────────────┐
│     Recurso/Tabla    │ Administra. │ Entrenador  │  Recepción  │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ users                │             │             │             │
│  - Ver (SELECT)      │     ✅      │     ✅      │     ✅      │
│  - Crear (INSERT)    │     ✅      │     ❌      │     ✅      │
│  - Editar (UPDATE)   │     ✅      │     ❌      │     ✅      │
│  - Eliminar (DELETE) │     ✅      │     ❌      │     ❌      │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ staff                │             │             │             │
│  - Ver               │     ✅      │     ❌      │     ❌      │
│  - Crear             │     ✅      │     ❌      │     ❌      │
│  - Editar            │     ✅      │     ❌      │     ❌      │
│  - Eliminar          │     ✅      │     ❌      │     ❌      │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ payments             │             │             │             │
│  - Ver               │     ✅      │     ✅      │     ✅      │
│  - Crear             │     ✅      │     ❌      │     ✅      │
│  - Editar            │     ✅      │     ❌      │     ✅      │
│  - Eliminar          │     ✅      │     ❌      │     ❌      │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ attendance           │             │             │             │
│  - Ver               │     ✅      │     ✅      │     ✅      │
│  - Registrar         │     ✅      │     ❌      │     ✅      │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ routine_templates    │             │             │             │
│  - Ver               │     ✅      │     ✅      │     ✅      │
│  - Crear             │     ✅      │     ✅      │     ❌      │
│  - Editar            │     ✅      │  ✅ (solo   │     ❌      │
│                      │             │   propias)  │             │
│  - Eliminar          │     ✅      │     ❌      │     ❌      │
├──────────────────────┼─────────────┼─────────────┼─────────────┤
│ physical_progress    │             │             │             │
│  - Ver               │     ✅      │     ✅      │     ✅      │
│  - Crear/Editar      │     ✅      │     ✅      │     ❌      │
└──────────────────────┴─────────────┴─────────────┴─────────────┘
```

---

## 📦 ESTRUCTURA DE ARCHIVOS

```
gimnasio-los-teques/
│
├── supabase/
│   ├── migrations/
│   │   └── schema.sql              ← Schema SQL completo (Ejecutar en Supabase)
│   │
│   └── functions/
│       └── server/
│           ├── index.tsx           ← Servidor Hono con endpoints
│           ├── seed.tsx            ← Datos de prueba
│           └── kv_store.tsx        ← [Sistema - No editar]
│
├── src/
│   ├── app/
│   │   ├── components/
│   │   │   ├── Sidebar.tsx
│   │   │   ├── StatCard.tsx
│   │   │   ├── DatabaseSetup.tsx  ← Componente para inicialización
│   │   │   └── ui/
│   │   │       ├── button.tsx
│   │   │       ├── card.tsx
│   │   │       └── ...
│   │   │
│   │   ├── lib/
│   │   │   ├── api.ts              ← Cliente API TypeScript
│   │   │   └── mockData.ts
│   │   │
│   │   ├── pages/
│   │   │   ├── Dashboard.tsx
│   │   │   ├── Users.tsx
│   │   │   ├── Payments.tsx
│   │   │   ├── Routines.tsx
│   │   │   ├── Staff.tsx
│   │   │   ├── Attendance.tsx
│   │   │   ├── TestSupabase.tsx   ← Página de testing
│   │   │   └── Layout.tsx
│   │   │
│   │   ├── types/
│   │   │   └── index.ts            ← TypeScript types
│   │   │
│   │   ├── routes.ts
│   │   └── App.tsx
│   │
│   └── styles/
│       ├── index.css
│       ├── theme.css
│       └── fonts.css
│
├── utils/
│   └── supabase/
│       └── info.tsx                ← Info de Supabase (projectId, key)
│
├── INSTRUCCIONES_SUPABASE.md       ← Guía paso a paso
├── GUIA_VISUAL_SUPABASE.md         ← Tutorial visual
├── SUPABASE_STRUCTURE.md           ← Estructura de BD
├── RESUMEN_CONFIGURACION.md        ← Resumen técnico
├── README_SUPABASE.md              ← Inicio rápido
├── ARQUITECTURA_SISTEMA.md         ← Este archivo
│
├── package.json
└── vite.config.ts
```

---

## 🗃️ MODELO DE DATOS

### Diagrama Entidad-Relación

```
┌─────────────┐
│    staff    │ ← Tabla de Personal con autenticación
│             │
│ id          │──┐
│ auth_user_id│  │
│ name        │  │
│ role        │  │ 1:N
│ email       │  │
│ phone       │  │
│ shift       │  │
│ status      │  │
└─────────────┘  │
                 │
                 ├─── created_by ───┐
                 │                  │
                 │                  ▼
                 │         ┌──────────────────┐
                 │         │ routine_templates│
                 │         │                  │
                 │         │ id               │──┐
                 │         │ name             │  │
                 │         │ level            │  │
                 │         │ category         │  │ 1:N
                 │         │ duration         │  │
                 │         └──────────────────┘  │
                 │                                │
                 ├─── assigned_by ───┐           │
                 │                   │           │
                 │                   ▼           ▼
┌─────────────┐  │         ┌───────────────────────────┐
│    users    │  │         │ user_routine_assignments  │
│ (miembros)  │  │         │                           │
│             │  │         │ id                        │
│ id          │──┤         │ user_id         ──────────┤
│ member_num  │  │         │ routine_id      ──────────┤
│ name        │  │         │ assigned_by     ──────────┘
│ email       │  │         │ start_date                │
│ phone       │  │         │ is_active                 │
│ status      │  │         └───────────────────────────┘
│ plan        │  │
│ weight      │  │
│ height      │  │
│ imc         │  │
└─────────────┘  │
      │          │
      │ 1:N      │
      │          │
      ├──────────┼──► ┌─────────────────┐
      │          │    │    payments     │
      │          │    │                 │
      │          │    │ id              │
      │          │    │ user_id         │
      │          │    │ amount          │
      │          │    │ status          │
      │          │    │ method          │
      │          │    └─────────────────┘
      │          │
      ├──────────┼──► ┌─────────────────┐
      │          │    │   attendance    │
      │          │    │                 │
      │          │    │ id              │
      │          │    │ user_id         │
      │          │    │ date            │
      │          │    │ type            │
      │          │    └─────────────────┘
      │          │
      └──────────┼──► ┌─────────────────┐
                 │    │physical_progress│
                 │    │                 │
                 │    │ id              │
                 │    │ user_id         │
                 │    │ weight          │
                 │    │ body_fat        │
                 │    └─────────────────┘
                 │
                 └──► ┌─────────────────┐
                      │workout_sessions │
                      │                 │
                      │ id              │──┐
                      │ user_id         │  │
                      │ routine_id      │  │ 1:N
                      │ date            │  │
                      │ is_completed    │  │
                      └─────────────────┘  │
                                           │
                                           ▼
                              ┌──────────────────────┐
                              │workout_exercise_logs │
                              │                      │
                              │ id                   │──┐
                              │ session_id           │  │
                              │ exercise_id          │  │ 1:N
                              │ is_completed         │  │
                              └──────────────────────┘  │
                                                        │
                                                        ▼
                                             ┌────────────────┐
                                             │   set_logs     │
                                             │                │
                                             │ id             │
                                             │ exercise_log_id│
                                             │ set_number     │
                                             │ reps           │
                                             │ weight         │
                                             └────────────────┘
```

---

## 🚀 TECNOLOGÍAS UTILIZADAS

### Frontend
- **React 18** - Framework UI
- **TypeScript** - Tipado estático
- **Tailwind CSS v4** - Estilos
- **React Router** - Navegación
- **Lucide React** - Iconos
- **Recharts** - Gráficas

### Backend
- **Supabase Edge Functions** - Serverless functions
- **Hono** - Framework web para Deno
- **Deno Runtime** - Runtime JavaScript/TypeScript
- **PostgreSQL** - Base de datos

### Autenticación y Seguridad
- **Supabase Auth** - Sistema de autenticación
- **JWT Tokens** - Autenticación basada en tokens
- **Row Level Security (RLS)** - Seguridad a nivel de BD
- **Políticas de PostgreSQL** - Control de acceso granular

---

## 🔄 CICLO DE VIDA DE UNA REQUEST

```
1. Usuario hace clic en botón
        ↓
2. Componente React llama a api.users.getAll()
        ↓
3. api.ts hace fetch con token del localStorage
        ↓
4. Request HTTPS a Supabase Edge Function
        ↓
5. Hono Server recibe la request
        ↓
6. Servidor verifica el token JWT
        ↓
7. Servidor consulta datos del usuario desde staff table
        ↓
8. Servidor hace query a PostgreSQL: SELECT * FROM users
        ↓
9. PostgreSQL aplica políticas RLS según rol del usuario
        ↓
10. PostgreSQL retorna solo los datos permitidos
        ↓
11. Servidor formatea la respuesta JSON
        ↓
12. Response HTTPS al frontend
        ↓
13. api.ts parsea el JSON
        ↓
14. Componente React recibe los datos
        ↓
15. React actualiza el DOM
        ↓
16. Usuario ve los datos en pantalla
```

---

## 📈 ESCALABILIDAD

### Puntos de Escalabilidad

| Componente | Escalabilidad | Notas |
|------------|---------------|-------|
| **Frontend** | ✅ Alta | Deploy en CDN (Vercel, Netlify) |
| **Edge Functions** | ✅ Alta | Serverless, escala automáticamente |
| **Database** | ✅ Media-Alta | PostgreSQL con conexión pooling |
| **Auth** | ✅ Alta | Supabase Auth escala automáticamente |

### Optimizaciones Implementadas

- ✅ **Índices en BD** - En columnas frecuentemente consultadas
- ✅ **Connection Pooling** - Supabase maneja el pool de conexiones
- ✅ **Lazy Loading** - Componentes React con code splitting
- ✅ **Caching** - localStorage para tokens y datos del usuario
- ✅ **Query Optimization** - Joins optimizados en PostgreSQL

---

## 🎯 PATRONES DE DISEÑO UTILIZADOS

### 1. Repository Pattern
```typescript
// Abstracción de acceso a datos
api.users.getAll()
api.payments.create()
api.stats.getDashboard()
```

### 2. Singleton Pattern
```typescript
// Cliente API único
import api from './lib/api';
```

### 3. Middleware Pattern
```typescript
// En el servidor Hono
app.use('*', logger(console.log));
app.use('/*', cors({ ... }));
```

### 4. Factory Pattern
```typescript
// Creación de requests con configuración
apiRequest('/users', { method: 'GET' })
```

---

## 🔮 FUTURAS MEJORAS SUGERIDAS

### Corto Plazo
- [ ] Implementar refresh tokens para sesiones largas
- [ ] Agregar paginación en listados grandes
- [ ] Implementar caché en frontend con React Query
- [ ] Agregar validación de formularios con Zod

### Mediano Plazo
- [ ] Implementar notificaciones push
- [ ] Agregar generación de reportes en PDF
- [ ] Implementar sistema de QR para asistencia
- [ ] Dashboard con gráficas avanzadas (Recharts)

### Largo Plazo
- [ ] App móvil con React Native
- [ ] Sistema de mensajería interno
- [ ] Integración con wearables (smartwatch)
- [ ] IA para recomendación de rutinas personalizadas

---

**Creado para:** Gimnasio Los Teques, Sector Lagunetica  
**Arquitectura diseñada:** Febrero 2026  
**Versión:** 1.0
