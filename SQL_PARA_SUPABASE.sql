-- =====================================================
-- 🏋️ GYM LAGUNETICA - SCHEMA COMPLETO
-- =====================================================
-- 
-- INSTRUCCIONES:
...
-- 8. ASIGNACIONES DE RUTINAS A USUARIOS
...
-- 10. LOGS DE EJERCICIOS EN SESIÓN
...
CREATE POLICY "Administradores pueden ver todos los perfiles"
  ...
  USING (auth.uid() IN (SELECT id FROM public.user_profiles WHERE role = 'administrador'));
...
CREATE POLICY "Administradores pueden crear perfiles"
  ...
  WITH CHECK (auth.uid() IN (SELECT id FROM public.user_profiles WHERE role = 'administrador'));
...
-- =====================================================
-- RLS POLICIES - physical_progress
-- =====================================================
CREATE POLICY "Entrenadores y admin pueden gestionar progreso"
  ON public.physical_progress FOR ALL
  USING (auth.uid() IN (SELECT id FROM public.user_profiles WHERE role IN ('administrador', 'entrenador')));
...
-- =====================================================
-- RLS POLICIES - routine_templates
-- =====================================================
...
CREATE POLICY "Entrenadores y admin pueden crear rutinas"
  ...
  WITH CHECK (auth.uid() IN (SELECT id FROM public.user_profiles WHERE role IN ('administrador', 'entrenador')));
...
CREATE POLICY "Todos pueden ver ejercicios de rutinas activas"
  ...
  USING (routine_id IN (SELECT id FROM public.routine_templates WHERE is_active = TRUE));
...
-- =====================================================
-- RLS POLICIES - user_routine_assignments
-- =====================================================
...
-- =====================================================
-- RLS POLICIES - workout_exercise_logs
-- =====================================================
